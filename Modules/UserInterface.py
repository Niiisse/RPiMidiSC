...
def updateUi():
    ...
    Ui.keysLastFrame = drawKeybinds(Ui.window, Ui.showKeyBinds, Ui.keysLastFrame,
                                    Ui.keyBinds)  # Draws the keyBindings, clears it only when necessary
    ...
    # EDITING MODE DRAWING #
        ...
        for y in range(5):
            for x in range(halfSteps):
                ...
                # Color selecting for first row
                if y < 2:
                    if sequencer.sets[sequencer.setIndex].patterns[sequencer.patternIndex].steps[
                        xStep].getState() == False:
